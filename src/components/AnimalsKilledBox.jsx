import React from "react";
import stats from "../data/stats_ytd.json";

function fmt(n) {
  if (typeof n !== "number") return n;
  if (n < 1_000) return String(n);
  if (n < 1_000_000) return (n / 1_000).toFixed(1).replace(/\.0$/, "") + "K";
  if (n < 1_000_000_000) return (n / 1_000_000).toFixed(1).replace(/\.0$/, "") + "M";
  if (n < 1_000_000_000_000) return (n / 1_000_000_000).toFixed(1).replace(/\.0$/, "") + "B";
  return (n / 1_000_000_000_000).toFixed(1).replace(/\.0$/, "") + "T";
}

export default function AnimalsKilledBox() {
  const { animalsKilledYTD, footnote } = stats;

  return (
    <div className="p-4 rounded-xl border border-gray-200 bg-white/70 shadow-sm">
      <h3 className="text-lg font-semibold text-gray-500 tracking-wide text-center mb-3">
  Animals Killed
  <span className="block font-normal text-gray-400 text-sm">(so far this year)</span>
</h3>



      <div className="grid grid-cols-2 gap-2">
        {animalsKilledYTD.map((item, i) => {
          const value =
            typeof item.min === "number" && typeof item.max === "number"
              ? `${fmt(item.min)} – ${fmt(item.max)}`
              : fmt(item.value);
          return (
            <div
              key={i}
              className="bg-gray-50 rounded-md p-2 text-center shadow-sm"
            >
              <div className="text-base font-bold text-[#265947] tabular-nums">
                {value}
              </div>
              <div className="text-[11px] text-gray-700 leading-tight">
                {item.label}
              </div>
            </div>
          );
        })}
      </div>
      {footnote && (
        <div className="mt-2 text-[10px] text-gray-500">{footnote}</div>
      )}
    </div>
  );
}
