...
import veganAthletes from "./data/vegan_athletes.json";

const VeganAthletes = () => {
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-2">Vegan Athletes</h1>
      ...
      <ul className="space-y-6">
        {veganAthletes.map((person, index) => (
          <li key={`${person.name}-${index}`} className="border-b border-gray-200 pb-4">
            <h2 className="text-xl font-semibold">{person.name}</h2>
            {person.sport && <p className="text-sm text-gray-600">{person.sport}</p>}
